...
- Faces bind by folder ID **and** the ID embedded in `face.fpk`; both must match or it renders default (created players included: folder copy alone is not enough). Equal-length IDs: `faces map` hex-swaps in place. Length-changing IDs (created-player `0x80000000`+ = 10 digits) need an FPK/FMDL repack; naive `ReplaceAll` corrupts offsets. Own relinker planned (public-source-based); until then use an external relinker.
