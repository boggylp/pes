...
description: '**Invoke this skill BEFORE proposing or running any switch of `dt13_all.cpk`, `dt18_all.cpk`, `FL_2026.exe`, or other gameplay-stack file on the live PES / Football Life install.** Covers `tools/fl-gameplay.ps1 switch-dt13-vanilla` / `switch-dt18-vanilla`, the canonical gameplay backup root at `%USERPROFILE%\MEGA\gaming\pes\gameplay\`, the SYSTEM cache scope rule (delete `SYSTEM00000000` only for dt13/dt18/exe changes, never for kits/cameras/luas, and only when the user explicitly says so), the no-partial-switch rule, and the explicit-user-approval gate. Triggers: "switch to vanilla dt18", "go back to stock", "restore dt13", "swap in alexfe87", "install F4L v5.1", any phrase that implies replacing a gameplay-stack file.'
---
...
3. **Ask the user to approve the exact change.** State, in one block:
    ...
    - whether SYSTEM cache invalidation is required (it is for dt13 / dt18 / exe; it is not for kits / cameras / luas, see [[feedback_system_cache_scope]])
    - whether the user wants to handle cache invalidation themselves (they always do, see below)
...
- **Never delete `SYSTEM00000000`** as part of the switch. The user always handles cache invalidation manually. The helper does not delete it, and neither should you. See [[feedback_system_cache_scope]].
